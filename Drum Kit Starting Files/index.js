var numberDrums = document.querySelectorAll(".drum").length;
for (var i = 0; i < numberDrums; i++) {

  document.querySelectorAll(".drum")[i].addEventListener("click", function() {

    var buttonSelector = this.innerHTML;

    makesound (buttonSelector);
    buttonAnimation(buttonSelector);

});
}
// this var emcompasses the whole switch item i guess

document.addEventListener("keydown", function(event) {

makesound(event.key);
buttonAnimation(event.key);
});


function makesound(key){
    switch (key) {
      case "w":
      var tom1 = new Audio('sounds/tom-1.mp3');

      // everything inbetween the case and break will be executed if the case is correct
      tom1.play();
/* the play part should be the same as the var */
        break;
        case "a":
        var tom2 = new Audio('sounds/tom-2.mp3');
  tom2.play();
  break;
  case "s":
  var tom3 = new Audio('sounds/tom-3.mp3');
tom3.play();
break;
case "d":
var tom4 = new Audio('sounds/tom-4.mp3');
tom4.play();
break;
case "j":
var crash = new Audio('sounds/crash.mp3');
crash.play();
break;
case "k":
var kick = new Audio('sounds/kick-bass.mp3');
kick.play();
break;
case "l":
var snare = new Audio('sounds/snare.mp3');
snare.play();
break;

      default: console.log(this.innerHTML);
      break;

    }
  }

  function buttonAnimation (currentKey){

    var activeButton = document.querySelector("."+ currentKey);
    activeButton.classList.add("pressed");
  setTimeout (function(){activeButton.classList.remove("pressed")},200);
  }






//   document.addEventListener("keydown", function (event){
//
// var event = this.keydown;
// switch keyfunc:
//
// case "w":
// var tom1 = new Audio('sounds/tom-1.mp3');
//
// // everything inbetween the case and break will be executed if the case is correct
// tom1.play();
// /* the play part should be the same as the var */
//   break;
//   case "a":
//   var tom2 = new Audio('sounds/tom-2.mp3');
// tom2.play();
// break;
// case "s":
// var tom3 = new Audio('sounds/tom-3.mp3');
// tom3.play();
// break;
// case "d":
// var tom4 = new Audio('sounds/tom-4.mp3');
// tom4.play();
// break;
// case "j":
// var crash = new Audio('sounds/crash.mp3');
// crash.play();
// break;
// case "k":
// var kick = new Audio('sounds/kick-bass.mp3');
// kick.play();
// break;
// case "l":
// var snare = new Audio('sounds/snare.mp3');
// snare.play();
// break;
//
// default: console.log(this.innerHTML);
// break;
//   })
//
//
//
//   //
